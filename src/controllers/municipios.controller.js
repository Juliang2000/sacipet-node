...
const obtenerPorIdUndeMunicipio = async(id_unde) => {
    try {
        let respuesta =
            await pool.query('SELECT id_codigo,descripcion FROM t_ubicaciones_geograficas WHERE t_ubicaciones_geograficas.id_unde=$1 AND t_ubicaciones_geograficas.vigente=true',[id_unde]);
        ...
        throw new Error(`Archivo municipios.controller.js -> obtenerTodos()\n${err}`);
    }
}

module.exports = { obtenerPorIdUndeMunicipio }